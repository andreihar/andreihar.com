...
const Page = async ({ params }: { params: { id: string; }; }): Promise<JSX.Element> => {
  ...
  return (
    <>
      {/* <SEO title={title} description={description} url={`project/${id}`} image={generateStorageImgUrl({ header: true, id: `${id}/banner` })} author="Andrei Harbachov" publishedDate={published.toISOString()} keywords={builtW?.join(', ')} section="Project" tags={builtW} /> */}
