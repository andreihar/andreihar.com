...
import { LikesCounter, ViewsAndLikesProvider, ViewsCounter } from '@/components/widgets/ViewsAndLikes';
      ...
      <Link href={`/blog/${id}` as any} className="block group">
        <div className="bg-white dark:bg-gray-900 rounded-xl shadow-md overflow-hidden group">
          <div className="relative overflow-hidden">
            <div className="flex flex-wrap gap-2 absolute top-2 start-2 z-10">
              {tags.map((tag, index) => (
          ...
          <div className="relative p-4 dark:text-white">
            <div className="relative">
              <Image alt={t('name', { f: forename, s: surname })} src={avatar} width={48} height={48} className="absolute top-0 transform -mt-10 w-12 h-12 rounded-full border-2 border-white z-10" />
            </div>
            <div className="text-xs text-gray-500 dark:text-gray-400 mt-6 mb-4">{format.dateTime(published, { year: 'numeric', month: 'long', day: 'numeric' })}</div>
            ...
            <div className="flex justify-end gap-4 mt-6 text-gray-500 dark:text-gray-400 text-xs">
              <div className="flex items-center">
                <HiOutlineClock className="w-5 h-5 me-1" />
                {time}
              </div>
              <div className="flex items-center">
                <HiOutlineEye className="w-5 h-5 me-1" />
                <ViewsCounter />
              </div>
              <div className="flex items-center">
                <HiOutlineThumbUp className="w-5 h-5 me-1" />
                <LikesCounter />
              </div>
            </div>
          </div>
